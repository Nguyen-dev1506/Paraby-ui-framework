...
Không dựa vào việc pytest tự chèn sys.path ngầm định qua sự có/không có `__init__.py` để import đúng — phải khai báo `pythonpath` tường minh trong `pyproject.toml`/`pytest.ini`. Mọi script chạy standalone (không qua pytest) phải tự thêm `sys.path` cần thiết, không giả định môi trường chạy giống pytest.
